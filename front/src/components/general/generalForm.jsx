...
const FormularioDinamico = ({ secoes, onSubmit, valoresIniciais = {}, onChangeCampo, onResult }) => {
      ...
      try {
        ...
        if (resultado.data.success) {
          toastRef.current.show({
            severity: 'success',
            summary: 'Sucesso',
            detail: resultado.data.success.mensagem,
          });
        }
      } catch (error) {
  ...
  return (
    <form className="p-fluid" onSubmit={handleSubmit}>
      ...
      <div className="col-12 mt-4">
        <Button style={{ background: 'var(--primary-color)' }} label="Enviar" type="submit" className="w-full border-none" />
      </div>
      <Toast ref={toastRef} />
    </form>
  );
